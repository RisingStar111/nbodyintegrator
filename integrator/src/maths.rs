...
// oop forgot that we just want to store the error
/// Essentially an f128 but architecture agnostic and onlu '128's for addition
pub struct CompensatedSummand {
    value: f64,
    error: f64,
}

impl CompensatedSummand {
    pub fn new(value: f64) -> Self {
        CompensatedSummand { value, error: 0. }
    }

    pub fn add(&mut self, value: f64) { // may need to force inline for simd
        let x = value - self.error;
        let t = self.value + x;
        self.error = (t - self.value) - x;
        self.value = t;
    }
}
