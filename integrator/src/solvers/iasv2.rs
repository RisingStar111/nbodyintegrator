...
// This implementation seems to be working at least, even if the sheet mod is making very little sense
// Tho still gives fairly different results to rebound (in terms of what timestep (not (just) due to to_com change?))... (and twice as slow)
// kept around to check against optimised version, as well as for easier possible float exact rebound recreation if wanted
    ...
    fn try_step(&mut self) -> Option<()> {
        ...
        if timestep_ratio < safety_factor { // including this abort leads to significantly different (assumedly better) results
            ...
            if self.last_delta_t != 0. {
                self.predict_next_step(timestep_ratio); // optimised version fixed to use updated b here
            }
