...
impl Solver for ParticleEuclidean {
    fn step(&mut self) {
        ...
        //         let delta = self.system.particles[j].position - self.system.particles[i].position;
        //         let sq_mag = delta.sq_mag();
        //         let a_mag = self.delta_t * self.system.constants.G / (sq_mag * sq_mag.sqrt());
        //         self.system.particles[i].velocity = self.system.particles[i].velocity + (delta * (a_mag * self.system.particles[j].mass));
        //         self.system.particles[j].velocity = self.system.particles[j].velocity + (delta * (a_mag * -self.system.particles[i].mass));   
        //     }
        // }
        // // drift
        // for particle in &mut self.system.particles {
        ...
        for i in 0..self.system.num_particles()-1 {
            ...
            for j in i+1..self.system.num_particles() {
                ...
                let sq_mag = delta.sq_mag();
                let a_mag = self.delta_t * self.system.constants.G / (sq_mag * sq_mag.sqrt());
                let g = crate::maths::fast_2sum(self.system.particles[i].velocity.x, (delta * (a_mag * self.system.particles[j].mass)).x + ex);
                self.system.particles[i].velocity.x = g.0;
                ex = g.1;
                // if i == 1 && ex != 0. {println!("{:?}", ex)}
                let g = crate::maths::fast_2sum(self.system.particles[i].velocity.y, (delta * (a_mag * self.system.particles[j].mass)).y + ey);
                self.system.particles[i].velocity.y = g.0;
                ey = g.1;
                let g = crate::maths::fast_2sum(self.system.particles[i].velocity.z, (delta * (a_mag * self.system.particles[j].mass)).z + ez);
                self.system.particles[i].velocity.z = g.0;
                ez = g.1;
                self.system.particles[j].velocity = self.system.particles[j].velocity + (delta * (a_mag * -self.system.particles[i].mass));   
            }
        }
