...
use crate::{maths, systems, traits::*};
...
impl ParticleIAS {
    ...
    fn predict_next_step(&mut self, timestep_ratio: f64) {
        ...
        for i in 0..self.b.p0.len() {
            ...
            self.guessed_b.p0[i] = q1*(self.b.p6[i]* 7.0 + self.b.p5[i]* 6.0 + self.b.p4[i]* 5.0 + self.b.p3[i]* 4.0 + self.b.p2[i]* 3.0 + self.b.p1[i]*2.0 + self.b.p0[i]);
            self.guessed_b.p1[i] = q2*(self.b.p6[i]*21.0 + self.b.p5[i]*15.0 + self.b.p4[i]*10.0 + self.b.p3[i]* 6.0 + self.b.p2[i]* 3.0 + self.b.p1[i]);
            self.guessed_b.p2[i] = q3*(self.b.p6[i]*35.0 + self.b.p5[i]*20.0 + self.b.p4[i]*10.0 + self.b.p3[i]* 4.0 + self.b.p2[i]);
            self.guessed_b.p3[i] = q4*(self.b.p6[i]*35.0 + self.b.p5[i]*15.0 + self.b.p4[i]* 5.0 + self.b.p3[i]);
            self.guessed_b.p4[i] = q5*(self.b.p6[i]*21.0 + self.b.p5[i]* 6.0 + self.b.p4[i]);
            self.guessed_b.p5[i] = q6*(self.b.p6[i]* 7.0 + self.b.p5[i]);
            self.guessed_b.p6[i] = q7* self.b.p6[i];
    ...
    fn try_step(&mut self) -> Option<()> { // breaks when particles have mass, in strange ways, somewhat aleviated by having a min_timestep
        ...
        let mut x0 = self.system.base.particles.iter().flat_map(|p| p.position.array()).collect::<Vec<f64>>();
        let mut v0 = self.system.base.particles.iter().flat_map(|p| p.velocity.array()).collect::<Vec<f64>>();
        let a0 = self.system.base.particles.iter().flat_map(|p| p.acceleration.array()).collect::<Vec<f64>>();
        ...
        let mut b = self.b.clone(); // todo stop cloning things
        // until converged
        ...
        loop {
            ...
            for (step, substep) in SUBSPACINGS[1..].iter().enumerate() { // maybe explictly unroll
                ...
                for i in 0..self.system.num_particles() {
                    ...
                    self.system.base.particles[i].position = 
        ...
        self.delta_t = (1e-9f64).max(min_timescale.sqrt() * old_delta_t * (self.epsilon*5040.).powf(1./7.)); // rebound has machine independent powf
        ...
        // println!("r, {:?}, {:?}, {:?}", timestep_ratio, old_delta_t, self.delta_t);
        if timestep_ratio < safety_factor { // including this abort leads to significantly different (assumedly better) results
            ...
            for i in 0..self.system.num_particles() {
                ...
                let y = 3*i+1;
                let z = 3*i+2;
                self.system.base.particles[i].position = [x0[x], x0[y], x0[z]].into();
                self.system.base.particles[i].velocity = [v0[x], v0[y], v0[z]].into();
                self.system.base.particles[i].acceleration = [a0[x], a0[y], a0[z]].into();
            }
        ...
        // set particles
        for i in 0..self.system.num_particles() {
            self.system.base.particles[i].position = crate::maths::V3 { x: x0[3*i], y: x0[3*i+1], z: x0[3*i+2] };
            self.system.base.particles[i].velocity = crate::maths::V3 { x: v0[3*i], y: v0[3*i+1], z: v0[3*i+2] };
        }
...
impl Solver for ParticleIAS {
    fn step(&mut self) {
        // go to heleocentric (ignoring error atm) // would only be able to do this once externally in rebound, less stable here than in try_step
        // self.system.to_com();
        // for i in (0..self.system.base.num_particles()).rev() { // why does this break it
        //     self.system.base.particles[i].position = self.system.base.particles[i].position - self.system.base.particles[0].position;
        //     self.system.base.particles[i].velocity = self.system.base.particles[i].velocity - self.system.base.particles[0].velocity;
        //     self.system.base.particles[i].acceleration = self.system.base.particles[i].acceleration - self.system.base.particles[0].acceleration;
        // }
